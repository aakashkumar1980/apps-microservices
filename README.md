...
4. **Create Controller Classes**
   ...
   - **Annotation Explanations:**<br/>  
     ...
     - `@GetMapping`, `@PostMapping`, `@PutMapping`, `@DeleteMapping` : Map HTTP methods to handler methods.
        > NOTE: "Idempotent" HTTP methods like GET, PUT, and DELETE does not duplicate the entity, while POST is can create duplicates if same body is used.
     - `ResponseEntity` : Represents the HTTP response, allowing you to set status codes and headers. e.g.:
       ...
       - (Create) POST > `ResponseEntity.status(HttpStatus.CREATED).header("Location", "/api/campaigns/1").body(campaign)` returns a 201 Created response with a Location header pointing to the newly created resource and the body serialized to JSON.<br/><br/>
